...
**Right answer: 60 seconds, not lower.** It's tempting to go lower — 30 seconds, even 15 — but the 60-second minimum resume charge punishes exactly that instinct against bursty, short-gap workloads: a warehouse set to suspend after 15 seconds, serving two 5-second queries only 20 seconds apart, suspends after the first query and pays a fresh 60-second minimum to resume for the second — 120 billed seconds total, plus two cold-cache query starts, for a 30-second total span (5s query + 20s gap + 5s query) it could have simply stayed up through. A 60-second suspend threshold would never have triggered on that 20-second gap in the first place, billing close to the ~30 seconds of real elapsed time instead of 120. Going lower doesn't save money against gaps shorter than the auto-suspend value — it fragments one cheap continuous span into multiple 60-second-minimum charges. (The comparison only flips the other way for genuinely long idle gaps — a warehouse sitting untouched for 10 minutes should suspend, obviously; the point of the 60-second floor is specifically about not suspending *faster than the minimum resume charge can pay for itself* against short, bursty gaps.)

**A second, independent reason not to go below 60 seconds: warehouse cache loss.** Suspending clears the warehouse's local result and metadata cache — every resume is a cold start for query performance, not just a billing event. A warehouse that suspends and resumes three times in ten minutes against a bursty dashboard workload is paying the billing cost above *and* serving every query post-resume without the cache benefit a less aggressive suspend setting would have preserved. The 60-second floor is where both costs — the billing minimum and the cache penalty — stop being worse than just tolerating a short idle gap.
...
- [Working with resource monitors — Snowflake Documentation](https://docs.snowflake.com/en/user-guide/resource-monitors)
