...
**The erasure caveat, resolved in `resilience_disaster_recovery.md`:** a `DELETE` in Snowflake doesn't make data unrecoverable immediately — it persists through the Time Travel retention window (up to 30 days for Regulated-tier data, per the tiered retention table in that doc) and then Fail-safe on top of that (a further fixed 7 days), up to 37 days worst case. The resolution isn't waiting out that window — it's crypto-shredding: Regulated-tier data subject to individual erasure requests is encrypted with a per-subject or per-tenant key at the application/pipeline layer before it lands in Snowflake, and a deletion request destroys that key. The ciphertext ages out of Time Travel/Fail-safe over the following weeks, but it's unreadable from the moment the key is destroyed — cryptographically equivalent to erased, immediately. See `resilience_disaster_recovery.md` — Closing the Privacy Doc's Erasure Dependency, for the full mechanism.
